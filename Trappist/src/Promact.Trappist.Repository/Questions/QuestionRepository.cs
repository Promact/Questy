...
namespace Promact.Trappist.Repository.Questions
{
    public class QuestionRepository : IQuestionRepository
    {
        ...
        public async Task<ICollection<Question>> GetAllQuestionsAsync(string userId, int id, int categoryId, string difficultyLevel, string searchQuestion)
        {
            ...
            if (!difficultyLevel.Equals("All"))
            {
                ...
                if (searchQuestion != null)
                    questionList = questionList.Where(x => x.DifficultyLevel == difficultyLevelCode && x.QuestionDetail.ToLowerInvariant().Contains(searchQuestion.ToLowerInvariant()));
                else
            ...
            return await questionList.Include(x => x.Category).Include(x => x.SingleMultipleAnswerQuestion).ThenInclude(x => x.SingleMultipleAnswerQuestionOption).OrderByDescending(x => x.CreatedDateTime).Skip(id * 10).Take(10).ToListAsync();
        }
